...
{
    if (s_panel_status)   lv_obj_move_foreground(s_panel_status);
    if (s_panel_response) lv_obj_move_foreground(s_panel_response);
    if (s_bar_progress)   lv_obj_move_foreground(s_bar_progress);
    if (s_label_progress) lv_obj_move_foreground(s_label_progress);
    if (s_panel_footer)   lv_obj_move_foreground(s_panel_footer);
}
                                  ...
                                  const char *text)
{
    ...
    lv_obj_set_style_bg_opa(lbl, LV_OPA_TRANSP, LV_PART_MAIN); /* Transparent label bg */
...
{
    if (!bsp_lvgl_lock(-1)) return ESP_FAIL;
    ...
    if (s_preview_buf) {
        lv_canvas_set_buffer(s_canvas_preview, s_preview_buf,
                             SCR_W, SCR_H, LV_IMG_CF_TRUE_COLOR);
    }

    /* -- Status bar (top, edge-to-edge) -- */
    s_panel_status = gui_create_strip(scr, &s_label_status, LV_ALIGN_TOP_LEFT, 0, "Pronto");
    ...
    lv_obj_set_style_bg_color(s_panel_response, lv_color_hex(C_RESPONSE_BG), LV_PART_MAIN);
    ...
    lv_obj_set_style_text_color(s_label_response, lv_color_hex(C_TEXT), LV_PART_MAIN);
    ...
    /* track (unfilled) */
    lv_obj_set_style_bg_color(s_bar_progress, lv_color_hex(C_BAR_TRACK), LV_PART_MAIN);
    ...
    /* indicator (filled) */
    lv_obj_set_style_bg_color(s_bar_progress, lv_color_hex(C_BAR_FILL), LV_PART_INDICATOR);
    ...
    lv_obj_set_style_text_color(s_label_progress, lv_color_hex(C_TEXT), LV_PART_MAIN);
    ...
    s_panel_footer = gui_create_strip(scr, &s_label_footer, LV_ALIGN_BOTTOM_LEFT, 0,
                                      "Enc:Falar Btn1:Foto Knob:Config");
...
{
    if (!s_label_status) return ESP_ERR_INVALID_STATE;
    if (!bsp_lvgl_lock(200)) return ESP_ERR_TIMEOUT;
    ...
    if (s_label_progress) lv_obj_add_flag(s_label_progress, LV_OBJ_FLAG_HIDDEN);
...
{
    if (!s_label_response || !s_panel_response) return ESP_ERR_INVALID_STATE;
    if (!bsp_lvgl_lock(200)) return ESP_ERR_TIMEOUT;
...
{
    if (!s_panel_response || !s_label_response) return ESP_ERR_INVALID_STATE;
    if (!bsp_lvgl_lock(200)) return ESP_ERR_TIMEOUT;
    ...
    if (compact) {
        ...
        lv_obj_set_style_bg_color(s_panel_response, lv_color_hex(0x202020), LV_PART_MAIN);
        lv_obj_set_style_bg_opa(s_panel_response, LV_OPA_COVER, LV_PART_MAIN);
        lv_obj_set_style_radius(s_panel_response, 8, LV_PART_MAIN);
        /* White text, bitmap font for sharp rendering. */
        lv_obj_set_style_text_color(s_label_response, lv_color_hex(0xFFFFFF), LV_PART_MAIN);
...
{
    if (!s_panel_response) return ESP_ERR_INVALID_STATE;
    if (!bsp_lvgl_lock(200)) return ESP_ERR_TIMEOUT;
...
{
    if (!s_label_response || !s_panel_response) return ESP_ERR_INVALID_STATE;
    if (delta_pixels == 0) return ESP_OK;
    if (!bsp_lvgl_lock(200)) return ESP_ERR_TIMEOUT;
...
{
    if (!s_label_footer) return ESP_ERR_INVALID_STATE;
    if (!bsp_lvgl_lock(200)) return ESP_ERR_TIMEOUT;
...
{
    if (!s_canvas_preview) return ESP_ERR_INVALID_STATE;
    if (!bsp_lvgl_lock(200)) return ESP_ERR_TIMEOUT;
                                         ...
                                         uint16_t width, uint16_t height)
{
    if (!rgb565_data || width == 0 || height == 0 ||
        !s_canvas_preview || !s_preview_buf) {
        if (s_canvas_preview) {
            if (!bsp_lvgl_lock(200)) return ESP_ERR_TIMEOUT;
            lv_obj_add_flag(s_canvas_preview, LV_OBJ_FLAG_HIDDEN);
            bsp_lvgl_unlock();
