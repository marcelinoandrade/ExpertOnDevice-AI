...
#define BSP_SD_SLOT_0_DEFAULT_INIT                                             \
  .clk = 0, .cmd = 0, .d0 = 0, .d1 = 0, .d2 = 0, .d3 = 0, .d4 = 0, .d5 = 0,    \
  .d6 = 0, .d7 = 0
 ...
 *
 * @note For LCD panel button which is defined as BSP_BUTTON_MAIN,
 * bsp_display_start should be called before call this function.
 *
 * @param[out] btn_array      Output button array
 * @param[out] btn_cnt        Number of button handlers saved to btn_array, can
 * be NULL
 * @param[in]  btn_array_size Size of output button array. Must be at least
 * BSP_BUTTON_NUM
 * @return
 ...
 *      - other error codes from SDMMC or SPI drivers, SDMMC protocol, or FATFS
 * drivers
 */
 ...
 * must take LVGL mutex by calling bsp_display_lock() before calling any LVGL
 * API (lv_...) and then give the mutex with bsp_display_unlock().
 *
 * If you want to use the display without LVGL, see bsp/display.h API and use
 * BSP version with 'noglib' suffix.
 **************************************************************************************************/
...
/**
 * @brief Draw a direct LCD color pattern using esp_lcd (without LVGL widgets).
 *
 * Useful for diagnosing cases where the backlight is ON but GUI content is not
 * visible.
 *
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_INVALID_STATE if the panel is not initialized yet
 */
 ...
 *  - MEMSensing Microsystems MSM261S4030H0: 48kHz, 24bit mono digital
 * microphone
 *
 * For microphone initialization use bsp_audio_codec_microphone_init() which is
 * inside initialize I2S with bsp_audio_init(). After microphone initialization,
 * use functions from esp_codec_dev for record audio. Example audio play:
 * \code{.c}
