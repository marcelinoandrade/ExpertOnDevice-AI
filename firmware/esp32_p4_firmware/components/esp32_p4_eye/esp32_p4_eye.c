 ...
 *
 * Can be used for i2s_pdm_rx_gpio_config_t and/or i2s_pdm_rx_config_t
 * initialization
 */
...
static const audio_codec_data_if_t *i2s_pdm_data_if =
...
static const st7789_lcd_init_cmd_t vendor_specific_init[] = {
    ...
    {0xE0,
     (uint8_t[]){0x70, 0x05, 0x0A, 0x0B, 0x0A, 0x27, 0x2F, 0x44, 0x47, 0x37,
                 0x14, 0x14, 0x29, 0x2F},
     14, 0},
    {0xE1,
     (uint8_t[]){0x70, 0x07, 0x0C, 0x08, 0x08, 0x04, 0x2F, 0x33, 0x46, 0x18,
                 0x15, 0x15, 0x2B, 0x2D},
     14, 0},
    {0x20, (uint8_t[]){0x00}, 1, 0}, // Display Inversion OFF
...
esp_err_t bsp_p4_eye_init(void) {
  ...
  ESP_ERROR_CHECK(esp_cam_sensor_xclk_allocate(
      ESP_CAM_SENSOR_XCLK_ESP_CLOCK_ROUTER, &xclk_handle));
  ESP_ERROR_CHECK(esp_cam_sensor_xclk_start(xclk_handle, &cam_xclk_config));

  const gpio_config_t sdcard_io_config = {.pin_bit_mask = BIT64(BSP_SD_EN_PIN),
  ...
  const gpio_config_t rst_io_config = {.pin_bit_mask =
                                           BIT64(BSP_CAMERA_RST_PIN),
                                       .mode = GPIO_MODE_OUTPUT,
                                       .pull_up_en = GPIO_PULLUP_DISABLE,
                                       .pull_down_en = GPIO_PULLDOWN_DISABLE,
                                       .intr_type = GPIO_INTR_DISABLE};
...
esp_err_t bsp_sdcard_mount(void) {
  ...
  // Configuracao identica ao factory demo funcional:
  // - SDMMC_FREQ_HIGHSPEED: necessario para o LDO operar corretamente
  // - width = 4: barramento 4-bit conforme hardware P4-EYE
  // - allocation_unit_size = 64KB: cluster FAT32 otimizado para fotos
  // - BSP_SD_SLOT_0_DEFAULT_INIT: no P4 os pinos slot 0 sao fixos no HW
  //   (nao usam GPIO matrix), entao os valores .clk/.cmd/.d0=0 sao ignorados
  //   pelo driver SDMMC que usa os pinos fisicos internos
  // NAO chamar sdmmc_host_deinit_slot(): o host SDMMC e compartilhado entre
  // slot 0 (SD) e slot 1 (ESP-Hosted/C6), causaria corrupcao do SDIO do C6.
  ...
  // Se o handle ja existe (static), nao tente criar um novo
  if (pwr_ctrl_handle == NULL) {
    ret = sd_pwr_ctrl_new_on_chip_ldo(&ldo_config, &pwr_ctrl_handle);
    if (ret != ESP_OK) {
      ESP_LOGE(TAG, "Failed to create LDO driver");
      return ret;
    }
  }

  // FORCE a voltagem diretamente no canal, ANTES do host SDMMC ligar
  ret = sd_pwr_ctrl_set_io_voltage(pwr_ctrl_handle, 3300);
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "Failed to set SD card voltage to 3.3V");
  }
  // O hardware precisa de tempo para o capacitor de saida carregar
  ...
  if (ret != ESP_OK) {
    ESP_LOGW(TAG, "SD mount failed (0x%x: %s), mas preservando LDO handle", ret,
             esp_err_to_name(ret));
  }
...
esp_err_t bsp_sdcard_detect_init(void) {
  const gpio_config_t sd_detect_config = {.pin_bit_mask =
                                              BIT64(BSP_SD_DETECT_PIN),
...
esp_err_t bsp_knob_register_cb(knob_event_t event, knob_cb_t cb,
                               void *usr_data) {
  if (knob == NULL) {
...
esp_err_t bsp_display_backlight_on(void) {
  /* Lower default brightness to reduce washed/saturated look on UI and preview.
   */
  return bsp_display_brightness_set(85);
}

esp_err_t bsp_display_fill_test_pattern(void) {
  if (!panel_handle) {
    return ESP_ERR_INVALID_STATE;
  }

  /* Draw full-screen solid colors directly via panel driver to isolate LVGL
   * issues. */
  static uint16_t line_buf[BSP_LCD_H_RES * 16];
  ...
  for (int repeat = 0; repeat < 2; repeat++) {
    for (size_t c = 0; c < sizeof(colors) / sizeof(colors[0]); c++) {
      for (size_t i = 0; i < (BSP_LCD_H_RES * 16); i++) {
        line_buf[i] = colors[c];
      }

      for (int y = 0; y < BSP_LCD_V_RES; y += 16) {
        const int y2 = (y + 16 <= BSP_LCD_V_RES) ? (y + 16) : BSP_LCD_V_RES;
        ESP_RETURN_ON_ERROR(esp_lcd_panel_draw_bitmap(panel_handle, 0, y,
                                                      BSP_LCD_H_RES, y2,
                                                      line_buf),
                            TAG, "panel draw failed");
      }
      ESP_LOGI(TAG, "LCD direct test color index=%u", (unsigned)c);
...
esp_err_t bsp_display_new(const bsp_display_config_t *config,
  ...
  if (io_config.pclk_hz != 20 * 1000 * 1000) {
    ESP_LOGW(TAG,
             "If the pixel clock is not set to 20 MHz, you need to temporarily "
             "apply the patch 0001-fix-spi-default-clock-source.patch. For "
             "details on how to apply the patch, please refer to the README.");
  }
  ESP_GOTO_ON_ERROR(
  ...
  // Send vendor-specific init commands AFTER panel init to override defaults
  const st7789_lcd_init_cmd_t *cmd = vendor_specific_init;
  uint16_t cmd_size =
      sizeof(vendor_specific_init) / sizeof(st7789_lcd_init_cmd_t);
  for (uint16_t i = 0; i < cmd_size; i++) {
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(*ret_io, cmd[i].cmd,
                                                  cmd[i].data,
                                                  cmd[i].data_bytes),
  ...
  // This ST7789 panel requires inversion ON for correct display
  // We'll compensate by inverting GUI colors
  esp_lcd_panel_invert_color(*ret_panel, true);

  // Send 0x21 (Display Inversion ON) command directly
  uint8_t inv_on = 0x21;
  ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(*ret_io, inv_on, NULL, 0), TAG,
                      "send 0x21 (inv on) failed");
...
esp_err_t bsp_flashlight_init(void) {
  const gpio_config_t led_io_config = {.pin_bit_mask =
                                           BIT64(BSP_LED_FLASHLIGHT),
                                       .mode = GPIO_MODE_OUTPUT,
                                       .pull_up_en = GPIO_PULLUP_DISABLE,
                                       .pull_down_en = GPIO_PULLDOWN_DISABLE,
                                       .intr_type = GPIO_INTR_DISABLE};
...
bool bsp_get_flashlight_status(void) { return led_status; }

esp_err_t bsp_extra_pdm_i2s_read(void *audio_buffer, size_t len,
...
esp_err_t bsp_pdm_audio_init(const i2s_pdm_rx_config_t *i2s_config) {
  ...
  /* Setup I2S channels */
  const i2s_pdm_rx_config_t pdm_cfg_default =
      BSP_I2S_PDM_DUPLEX_MONO_CFG(16000);
  ...
  if (i2s_pdm_rx_chan != NULL) {
    ESP_ERROR_CHECK(
        i2s_channel_init_pdm_rx_mode(i2s_pdm_rx_chan, p_i2s_pdm_cfg));
