...
#define AP_SSID "Assistant-Config-P4"
...
static const char *s_html_page =
    ...
    "<title>Assistant-P4 Config</title>"
    "<style>"
...
esp_err_t captive_portal_start(void) {
   ...
   * ---------------------------------------------------------------- */
  esp_err_t err = esp_wifi_remote_set_mode(WIFI_MODE_AP);
  if (err != ESP_OK) {
  ...
  err = esp_wifi_remote_set_config(WIFI_IF_AP, &ap_cfg);
  if (err != ESP_OK) {
  ...
  if (err != ESP_OK && err != ESP_ERR_WIFI_CONN) {
    ESP_LOGE(TAG, "wifi_remote_start(AP) failed: %s", esp_err_to_name(err));
    return err;
  }
