...
static int app_queue_count_locked(void) { return s_queue_count; }
...
static void app_storage_process_queue_internal(void) {
  ...
  // Processa fila: salva todas as imagens
  int saved_count = 0;
  int failed_count = 0;
...
esp_err_t app_storage_save_image(const uint8_t *jpeg_data, size_t jpeg_len) {
  ...
  snprintf(filename, sizeof(filename), "%s/IMG_%04d%02d%02d_%02d%02d%02d.jpg",
           SD_IMAGES_PATH, timeinfo.tm_year + 1900, timeinfo.tm_mon + 1,
...
esp_err_t app_storage_save_audio(const uint8_t *pcm_data, size_t pcm_bytes,
  ...
  snprintf(filename, sizeof(filename), "%s/REC_%04d%02d%02d_%02d%02d%02d.wav",
           SD_MEDIA_PATH "/audio", ti.tm_year + 1900, ti.tm_mon + 1, ti.tm_mday,
...
esp_err_t app_storage_save_chat_log(const char *mode_label,
  ...
  snprintf(filename, sizeof(filename), "%s/CHAT_%04d%02d%02d.txt",
           SD_BASE_PATH "/logs/chat", ti.tm_year + 1900, ti.tm_mon + 1,
           ti.tm_mday);
